    ...
    <div>
      ...
      <ul className="space-y-2">
        {shown.slice(0, 60).map((w) => {
          ...
          return (
            <li key={`${w.surah}:${w.i}`} className="flex items-center justify-between gap-3 rounded-lg bg-white px-3 py-2 ring-1 ring-ink/5">
              <div className="min-w-0">
                <div className="ayah text-2xl leading-tight text-ink" dir="rtl">
                  {w.uthmani}
                </div>
                <div className="truncate text-xs text-ink/50">
                  <span className={w.count >= 3 ? "font-semibold text-red-600" : "font-semibold text-amber-700"}>
                    {w.count}×
                  </span>
                  {" · "}
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                <button
                  ...
                  className="rounded-full px-2.5 py-1 text-xs font-semibold text-emerald ring-1 ring-emerald/30 hover:bg-emerald/5"
                >
                  ▶ Hear
                </button>
...
function FilterChip({ on, onClick, label }: { on: boolean; onClick: () => void; label: string }) {
  return (
    <button
      onClick={onClick}
      className={`rounded-full px-2.5 py-1 font-medium ring-1 transition ${on ? "bg-emerald text-white ring-emerald" : "bg-white text-ink/70 ring-ink/15 hover:bg-ink/5"}`}
    >
      {label}
    </button>
  );
