    ...
    <div>
      ...
      <ul className="space-y-2">
        {shown.slice(0, 60).map((w) => {
          ...
          return (
            <li key={`${w.surah}:${w.i}`} className="flex items-center justify-between gap-3 rounded-lg bg-surface-2 px-3 py-2 ring-1 ring-white/5">
              <div className="min-w-0">
                <div className="ayah text-2xl leading-tight text-ink" dir="rtl">
                  {w.uthmani}
                </div>
                <div className="truncate text-xs text-ink/50">
                  <span className={w.count >= 3 ? "font-semibold text-red-400" : "font-semibold text-amber-300"}>
                    {w.count}×
                  </span>
                  {" · "}
...
function FilterChip({ on, onClick, label }: { on: boolean; onClick: () => void; label: string }) {
  return (
    <button
      onClick={onClick}
      className={`rounded-full px-2.5 py-1 font-medium ring-1 transition ${on ? "bg-emerald text-white ring-emerald" : "bg-surface-2 text-ink/70 ring-white/15 hover:bg-white/5"}`}
    >
      {label}
    </button>
  );
