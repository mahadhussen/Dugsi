...
export default function ProgressPanel() {
  ...
  return (
    <div className="rounded-2xl border border-ink/10 bg-surface p-5 shadow-soft backdrop-blur-sm sm:p-6">
      ...
      {needsWork.length > 0 && (
        <div className="mt-5">
          ...
          <ul className="space-y-2">
            {needsWork.map((s) => (
              <li key={s.surah} className="flex items-center justify-between rounded-xl border border-ink/10 bg-surface-2 px-3 py-2">
...
function GoalRing({ pct, met }: { pct: number; met: boolean }) {
  const r = 22;
  const c = 2 * Math.PI * r;
  const color = met ? "var(--good)" : "#cfae5e";
  return (
