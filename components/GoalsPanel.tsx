  ...
  return (
    <ul className="space-y-3">
      {goals.map((g) => {
        ...
        return (
          <li key={g.key} className="rounded-xl border border-ink/10 bg-white p-3">
            <div className="flex items-center justify-between gap-2 text-sm">
              <span className="text-ink/80">
                {g.label} <span className="text-xs text-ink/45">· {g.period}</span>
              </span>
              <span className="flex items-center gap-2">
                <span className={`font-semibold ${met ? "text-emerald-deep" : "text-ink"}`}>
                  {g.done}
            ...
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-ink/10">
              <div className="h-full rounded-full transition-all" style={{ width: `${Math.max(2, pct)}%`, backgroundColor: met ? "#0f766e" : "#c9a24b" }} />
            </div>
          </li>
        );
      })}
