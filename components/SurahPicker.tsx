              ...
              {filtered.map((s) => (
                <li key={s.id}>
                  <button
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-2.5 text-left transition hover:bg-emerald/10 ${
                      s.id === current ? "bg-emerald/10" : ""
                    }`}
                  >
                    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-gold/15 text-xs font-semibold text-gold-soft">
                      {s.id}
                    </span>
