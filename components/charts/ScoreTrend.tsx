  ...
  return (
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} className="block h-24 w-full" role="img" aria-label="Score trend">
        {[90, 60].map((g) => (
          <line key={g} x1={padX} x2={w - padX} y1={y(g)} y2={y(g)} stroke="#10241f" strokeOpacity="0.08" strokeDasharray="2 3" />
        ))}
        <path d={path} fill="none" stroke="#0f766e" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
        {points.map((p, i) => (
          <circle key={i} cx={x(i)} cy={y(p.score)} r={i === n - 1 ? 4 : 3} fill={i === n - 1 ? "#0f766e" : "#faf6ec"} stroke="#0f766e" strokeWidth="1.5">
            <title>
      ...
      <p className="mt-1 text-xs text-ink/55">
        ...
        {delta !== 0 && (
          <span className={delta > 0 ? "text-emerald-deep" : "text-amber-700"}>
            {" "}
            ({delta > 0 ? "+" : ""}
